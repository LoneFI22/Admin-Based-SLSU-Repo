...
<body>
    ...
    <div class="content">
        ...
        <main>
            ...
            <input type='hidden' id='idOfUser' value='<?php echo isset($_GET['user_id']) ? $_GET['user_id'] : ""; ?>'>
            <input type='hidden' id='idOfAdmin' value='<?php echo isset($_SESSION["admin_id"]) ? $action->getId($_SESSION["admin_id"]) : ""; ?>'>

            <input type="hidden" name="">
            <div class="userProfile">
                <?php
                if(isset($_GET['user_id'])){
                    $data = $action->getUser($_GET['user_id']);
                ?>
                <div class="row">
                    <div class="box-box col s12 m4">
                        <div class="box-container container grey lighten-5">
                            ...
                            <form method="post" class="align-button" id="imageUploadForm" enctype="multipart/form-data">
                                <input id="userId" type="hidden" name="userId" style="display: none;" value="<?= $_GET['user_id']?> ">
                                <input id="profile-pic-upload" type="file" name="imageProfile" accept="image/*"style="display: none;">
                                <button class="btn waves-effect waves-light" type="submit" id="uploadButton">Upload</button>
                            </form>
                        </div>
                    </div>
                                ...
                                <div id="edit" class="col s12">
                                    <form action="" method="post" id="editUserForm">
                                        <div class="row">
                                            ...
                                            <button type="button" class="editUserInfoButton waves-effect waves-light btn" style="padding-left: 30px; padding-right: 30px">Save</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
    ...
    <script>
        function compareId() {
            const user_id = $('#idOfUser').val();
            const admin_id = $('#idOfAdmin').val();

            const resId = user_id !== '' ? user_id : admin_id;
            return resId;
        }
        $(document).ready(function(){
            ...
            function infoLoad(user_id){
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        $('.info').empty();
                        $('.info').html(infoLoad);
                        $('.info input, .info textarea').each(function(){
                            M.updateTextFields();
                        });
                    }
                });
            ...
            function editInfo(user_id){
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        infoLoad += '</div>';
                        $('.edit_info').empty();
                        $('.edit_info').html(infoLoad);
                        $('.edit_info input, .edit_info textarea').each(function(){
                            M.updateTextFields();
                        });
                    }
                });
            ...
            $('.editInfoButton').click(function(){
                var formData = {editInfo: 'editInfo',};
            ...
            $('.editUserInfoButton').click(function(){
                ...
                $('#editUserForm').find('input, textarea, select').each(function(index, element) {
                    var field = $(element);
                    formData[field.attr('id')] = field.val();
                });
                $.ajax({
